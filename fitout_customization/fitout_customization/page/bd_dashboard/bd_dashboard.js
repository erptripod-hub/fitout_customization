frappe.pages["bd-dashboard"].on_page_load = function(wrapper) {
	...
	page.main.html(`
		<style>
			.bdash{background:#f0f2f8;min-height:100vh;padding:0;font-family:sans-serif}
			...
			.bdash .hlbl{font-size:10px;font-weight:700;text-transform:uppercase;margin-top:2px;color:#7b8aab}
			...
			.bdash .loading{text-align:center;padding:60px;color:#7b8aab;font-size:14px}
		</style>
		<div class="bdash">
			<div class="hdr">
				<div>
					<h1>TRIPOD MENA | <em>BD Performance Dashboard</em></h1>
	...
	function render(d) {
		...
		var ownerRows=(d.by_owner||[]).length?(d.by_owner||[]).sort(function(a,b){return b.value-a.value;}).map(function(o,i){
			...
			return '<tr><td><div style="display:flex;align-items:center;gap:8px"><div class="av" style="background:'+colors[i%colors.length]+'">'+ini+'</div><strong>'+o.name.split("@")[0]+'</strong></div></td><td>'+(ann?fmt(ann):"—")+'</td><td style="color:#0d9488;font-weight:600">'+fmt(o.value)+'</td><td>'+o.total+'</td><td>'+o.qualified+'</td><td>'+o.converted+'</td><td>'+(ann?'<div style="display:flex;align-items:center;gap:6px"><div style="width:60px;height:6px;background:#f0f2f8;border-radius:3px;overflow:hidden"><div style="width:'+pr+'%;height:100%;border-radius:3px;background:'+col+'"></div></div><span style="font-size:11px;font-weight:700;color:'+col+'">'+pr+'%</span></div>':"—")+'</td></tr>';
		...
		document.getElementById("bd-main").innerHTML=
			'<div class="krow">'+
			'<div class="kcard" style="border-left-color:#2563eb"><div class="klbl">Total Leads</div><div class="kval">'+s.total+'</div><div class="ksub">All BD leads</div></div>'+
			'<div class="kcard" style="border-left-color:#16a34a"><div class="klbl">Qualified</div><div class="kval">'+s.qualified+'</div><div class="ksub" style="color:#16a34a">'+pct(s.qualified,s.total)+'% rate</div></div>'+
			'<div class="kcard" style="border-left-color:#7c3aed"><div class="klbl">Converted</div><div class="kval">'+s.converted+'</div><div class="ksub" style="color:#16a34a">'+pct(s.converted,s.total)+'% rate</div></div>'+
			'<div class="kcard" style="border-left-color:#dc2626"><div class="klbl">Lost</div><div class="kval">'+s.lost+'</div><div class="ksub" style="color:#dc2626">'+pct(s.lost,s.total)+'% rate</div></div>'+
			...
			'<div class="hgrid">'+
			'<div class="hcard ac"><div class="hnum">'+h.active+'</div><div class="hlbl">Active</div><div style="font-size:10px;color:#16a34a;margin-top:2px">under 30 days</div></div>'+
			...
			'<div class="hcard gh"><div class="hnum">'+h.ghost+'</div><div class="hlbl">Ghost</div><div style="font-size:10px;color:#dc2626;margin-top:2px">60+ days</div></div>'+
			'</div>'+
			'<div class="divdr"></div>'+
