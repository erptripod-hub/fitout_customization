...
def get_dashboard_data(year=None, salesperson=None):
    ...
    # Only leads with BD Status filled = real pipeline
    filters = {
    ...
    # All leads count separately for context
    total_all = frappe.db.count("Lead", {"status": ["!=", "Do Not Contact"]})
    ...
    for l in leads:
        ...
        owner_data[o]["weighted"] += val * ((l.custom_probability or 0) / 100)
        owner_data[o]["margin"] += val * ((l.custom_estimated_margin_pct or 0) / 100)
    ...
    ghost_leads = frappe.get_all("Lead",
        filters={"custom_ghost_status": ["in", ["Ghost", "Stale"]], "custom_lead_status": ["!=", ""]},
